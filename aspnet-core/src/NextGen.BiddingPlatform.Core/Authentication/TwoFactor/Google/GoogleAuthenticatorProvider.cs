...
namespace NextGen.BiddingPlatform.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : BiddingPlatformServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("NextGen.BiddingPlatform", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
