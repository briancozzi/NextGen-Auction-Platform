...
namespace NextGen.BiddingPlatform.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : BiddingPlatformAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<BiddingPlatformDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{BiddingPlatformConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{BiddingPlatformConsts.ConnectionStringName}", constring);
        }
